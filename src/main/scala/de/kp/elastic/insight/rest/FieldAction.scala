...
class FieldAction @Inject()(settings:Settings,client:Client,controller:RestController) extends InsightRestHandler(settings, client) {
   ...
   * 
   */
  private val requestBuilder  = new SimpleRequestBuilder("fields")
  private val responseBuilder = new FieldResponseBuilder()
  
  override protected def handleRequest(request:RestRequest,channel:RestChannel,client:Client) {

    try {

      logger.info("FieldAction: Request received")
